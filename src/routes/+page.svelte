<script>
	let textToBeStupified = '';
	let stupifiedResponse = 'NoT StUpIfIeD yEt';
	async function stupifyRequest() {
		if (textToBeStupified.trim().length === 0) return;
		console.log(JSON.stringify({ textToBeStupified }));
		const response = await fetch('/api/stupify', {
			method: 'POST',
			body: JSON.stringify({ textToBeStupified }),
			headers: {
...
<div class="container h-full mx-auto  flex justify-center items-center">
	<div class="space-y-10 text-center">
		<h2 class="font-bold">So they said something stupid again huh? Let's make it appropriate</h2>
		<div>
			<textarea
				bind:value={textToBeStupified}
				placeholder="Write the text you want to stupify here"
				class="min-w-fit w-80 min-h-fit h-20 p-6 text-black"
			/>
		</div>
		<!-- / -->
		<div class="flex justify-center space-x-2">
			<figure>
				...
				<svg class="fill-token w-32 h-32" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200">
					<path
						fill-rule="evenodd"
			...
			<button on:click={stupifyRequest} class="btn btn-filled w-40 h-12 mt-12 mx-auto"
				>S t U p I f Y</button
			>
		</div>
		<div class="card min-w-fit min-h-fit h-20 p-6 bg-secondary-400">{stupifiedResponse}</div>
	</div>
</div>
...
<style lang="postcss">
	...
	figure svg,
	.img-bg {
		@apply w-32 h-32 md:w-32 md:h-32;
	}
	.img-bg {
		@apply bg-gradient-to-r from-primary-400 to-warning-100;
